using UnityEngine;
using System.Collections;

public class MovingWhale : MonoBehaviour {
	
	public float hight = 3f;
	
	void Update() {
		//move the cube from (0, 0, 0) to (5, 0, 0) and back to (0, 0, 0) and so on.
		//use the same Mathf.PingPong function to the other axes if you need to move in those axes as well
		transform.position = new Vector3(transform.position.x, Mathf.PingPong(Time.time * 1, hight) - hight/2f, transform.position.z);//move on y axis only
	}
	
	void OnTriggerEnter2D(Collider2D other)
	{

	}


}